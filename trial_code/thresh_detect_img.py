...
def main():
    ...
    while True:
        ...
        red_result, red_mask = detect_color(image, red_lower, red_upper, color_space='HSV')

        # Threshold to detect blue color in HSV
        blue_result, blue_mask = detect_color(image, blue_lower, blue_upper, color_space='HSV')

        # Threshold to detect purple color in HSV
        purple_result, purple_mask = detect_color(image, purple_lower, purple_upper, color_space='HSV')

        # Threshold to detect color in grayscale
        gray_lower = np.array([gray_lower])
        gray_upper = np.array([gray_upper])
        gray_result, gray_mask = detect_color(image, gray_lower, gray_upper, color_space='GRAY')
